    ...
    const buttons = [
        {
            ...
            onclick: () => {
                binaryTreeRef.current.inOrder(binaryTreeRef.current.root, send, canvasRef);
            },
        },
        {
            label: "PREORDER TRAVERSAL",
            onclick: () => {
                binaryTreeRef.current.preOrder(binaryTreeRef.current.root, send, canvasRef);
            },
        },
        {
            label: "POSTORDER TRAVERSAL",
            onclick: () => {
                binaryTreeRef.current.postOrder(binaryTreeRef.current.root, send, canvasRef);
            },
        },
        {
            label: "INVERT BINARY TREE",
            onclick: () => {
                binaryTreeRef.current.inverted(binaryTreeRef.current.root, canvasRef, 0, canvasRef.current.width);
    ...
    return (
        <div id="dsa-container">
                ...
                <div id="inner-left">
                    ...
                    <div id="input-containers">
                        ...
                        <div className="input-button">
                            ...
                            <button onClick={() => {}} style={{ backgroundColor: "#e74c3c" }}>
                                <DeleteIcon />
                            </button>
                        </div>
                    ...
                    <div id="legend">
                        ...
                        {legend.map((e, index) => {
                            return (
                                <div key={index} className="legend-color" id={`legend-color-${index}`}>
                                    <div></div>
                                    <p>{e}</p>
                                </div>
                            );
            ...
            <div id="dsa-container-right">
                ...
                <canvas ref={canvasRef} height={"800px"} width={"1450px"}></canvas>
                {traversalType && (
                    <div id="output-board">
                        {traversalOutput?.map((e, index) => {
                            return (
                                <p key={index}>
                                    {e} {traversalOutput.length - 1 !== index && <span>--&gt;</span>}
                                </p>
                            );
                        })}
                    </div>
